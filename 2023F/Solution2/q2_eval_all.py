...
def eval_all():
    ...
    all_scores.to_csv(f"2023F/Solution2/results/{height}_{threshold_abs}.csv")
    
def cal_score():
    df = pd.read_csv("2023F/Solution2/results/20_7km_35.csv")
    print(df["mse"].mean(), df["ssim"].mean(), df["pod"].mean(), df["far"].mean(), df["csi"].mean())   
