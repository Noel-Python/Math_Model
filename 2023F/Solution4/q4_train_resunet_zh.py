...
def trainer():
    ...
    for epoch in tqdm.tqdm(range(1, num_epochs + 1)):
        ...
        if epoch % 10 == 0:
            torch.save(FE_Zh.state_dict(), f"2023F/Solution4/zh/FE_{height}_Zh_{epoch}.pth")
            torch.save(FC.state_dict(), f"2023F/Solution4/zh/FC_{height}_rain_{epoch}.pth")
            df = pd.DataFrame({"MSE loss": loss_list})
            df.to_csv(f"2023F/Solution4/zh/Train_MSE_loss_{height}.csv")
