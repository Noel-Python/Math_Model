...
def trainer():
    ...
    for epoch in tqdm.tqdm(range(1, num_epochs + 1)):
        ...
        for i, contents in enumerate(dataloader):
            ...
            feats_total = [torch.cat((feats_Zh[i], feats_Kdp[i]), dim=1) for i in range(8)]
        ...
        if epoch % 10 == 0:
            torch.save(FE_Zh.state_dict(), f"2023F/Solution4/zhkdp/FE_{height}_Zh_{epoch}.pth")
            torch.save(FE_Kdp.state_dict(), f"2023F/Solution4/zhkdp/FE_{height}_Kdp_{epoch}.pth")
            torch.save(FC.state_dict(), f"2023F/Solution4/zhkdp/FC_{height}_rain_{epoch}.pth")
            df = pd.DataFrame({"MSE loss": loss_list})
            df.to_csv(f"2023F/Solution4/zhkdp/Train_MSE_loss_{height}.csv")
