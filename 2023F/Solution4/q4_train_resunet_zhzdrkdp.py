...
def trainer():
    ...
    for epoch in tqdm.tqdm(range(1, num_epochs + 1)):
        ...
        if epoch % 10 == 0:
            torch.save(FE_Zh.state_dict(), f"2023F/Solution4/zhzdrkdp/FE_{height}_Zh_{epoch}.pth")
            torch.save(FE_Kdp.state_dict(), f"2023F/Solution4/zhzdrkdp/FE_{height}_Kdp_{epoch}.pth")
            torch.save(FE_Zdr.state_dict(), f"2023F/Solution4/zhzdrkdp/FE_{height}_Zdr_{epoch}.pth")
            torch.save(FC.state_dict(), f"2023F/Solution4/zhzdrkdp/FC_{height}_rain_{epoch}.pth")
            df = pd.DataFrame({"MSE loss": loss_list})
            df.to_csv(f"2023F/Solution4/zhzdrkdp/Train_MSE_loss_{height}.csv")
